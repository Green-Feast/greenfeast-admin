...
export default async function BatchesPage() {
  const [batchRes, partnersRes, subsRes] = await Promise.all([
    supabaseAdmin
      .from("batches")
      .select(`
        id, name, area, time_window, meal_slot, primary_partner_id, secondary_partner_id,
    ...
    supabaseAdmin
      .from("subscriptions")
      .select("id, user_id, status, plan_name, batch_id_lunch, batch_id_dinner, meals_lunch, meals_dinner, users!inner ( name, phone )")
  ...
  const batches = (batchRes.data ?? []).map((b: any) => ({
    ...
    meal_slot: b.meal_slot as "lunch" | "dinner",
  ...
  const subscribers = Array.from(bestSub.values()).map((s: any) => {
    ...
    return {
      ...
      plan: s.plan_name ?? "—",
      batchIdLunch: s.batch_id_lunch as string | null,
      batchIdDinner: s.batch_id_dinner as string | null,
      mealsLunch: (s.meals_lunch ?? 0) as number,
      mealsDinner: (s.meals_dinner ?? 0) as number,
      city: "",
    }
  })
